...
from s3_utils import s3_client, BUCKET_NAME   # 👈 add kiya
...
# 👇 New endpoint: fetch image from S3
@router.get("/blogs/{blog_id}/image")
async def get_blog_image(blog_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        key = blog.image_url.split("/")[-1]  # filename extract
